...
#define CG_HAS_AVX2 1
#include <immintrin.h>
#include <omp.h>
#include <limits.h>
#endif

#if defined(CG_HAS_AVX2)
...
/**
 * Update p = src + beta * p using AVX2 with OpenMP
...
static void cg_avx2_destroy(poisson_solver_t* solver) {
...
/**
 * CG AVX2 solve function (supports both standard CG and preconditioned CG)
 *
 ...
 *                                   rho = rho_new
 */
static cfd_status_t cg_avx2_solve(
    poisson_solver_t* solver,
    double* x,
    double* x_temp,
    const double* rhs,
    poisson_solver_stats_t* stats)
{
    ...
    for (iter = 0; iter < params->max_iterations; iter++) {
        ...
        if (iter % params->check_interval == 0) {
            if (params->verbose) {
                CFD_LOG_DEBUG("poisson", "CG AVX2 Iter %d: residual = %.6e", iter, res_norm);
            }
...
static cfd_status_t cg_avx2_iterate(
    poisson_solver_t* solver,
    double* x,
    double* x_temp,
    const double* rhs,
...
poisson_solver_t* create_cg_avx2_solver(void) {
    ...
    solver->init = cg_avx2_init;
    solver->destroy = cg_avx2_destroy;
    solver->solve = cg_avx2_solve;
    solver->iterate = cg_avx2_iterate;
