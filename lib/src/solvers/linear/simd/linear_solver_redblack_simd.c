/**
 * @file linear_solver_redblack_simd.c
 * @brief Red-Black SOR solver - SIMD (AVX2) implementation
 *
...
typedef struct {
    ...
    int initialized;
} redblack_simd_context_t;

/* ============================================================================
 * RED-BLACK SIMD IMPLEMENTATION (AVX2)
 * ============================================================================ */

static cfd_status_t redblack_simd_init(
    ...
    const poisson_solver_params_t* params)
{
    ...
    redblack_simd_context_t* ctx = (redblack_simd_context_t*)cfd_aligned_calloc(1, sizeof(redblack_simd_context_t));
    if (!ctx) {
...
static void redblack_simd_destroy(poisson_solver_t* solver) {
...
static cfd_status_t redblack_simd_iterate(
    poisson_solver_t* solver,
    double* x,
    double* x_temp,
    const double* rhs,
    double* residual)
{
    ...
    redblack_simd_context_t* ctx = (redblack_simd_context_t*)solver->context;
    size_t nx = solver->nx;
    size_t ny = solver->ny;
    double dx2 = ctx->dx2;
    double dy2 = ctx->dy2;
...
poisson_solver_t* create_redblack_simd_solver(void) {
    ...
    solver->description = "Red-Black SOR iteration (AVX2 SIMD)";
    solver->method = POISSON_METHOD_REDBLACK_SOR;
    solver->backend = POISSON_BACKEND_SIMD;
    solver->params = poisson_solver_params_default();

    solver->init = redblack_simd_init;
    solver->destroy = redblack_simd_destroy;
    solver->solve = NULL;
    solver->iterate = redblack_simd_iterate;
