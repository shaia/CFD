 ...
 * (see cpu/multigrid_transfer.c). Grid dimensions must be 2^k+1 per active
 * dimension.
 *
 ...
 */
static void mg_apply_bc_level(const mg_level_t* L, mg_bc_type_t bc, double* x) {
...
/**
 * Red-Black Gauss-Seidel smoother (omega = 1: over-relaxation degrades the
 * high-frequency smoothing factor, so plain GS is correct inside multigrid).
 * Same stencil algebra as linear_solver_redblack.c. BCs after each sweep.
 */
static void mg_smooth_rbgs(const mg_level_t* L, mg_bc_type_t bc,
    ...
    for (int sweep = 0; sweep < sweeps; sweep++) {
        ...
        mg_apply_bc_level(L, bc, x);
    }
}

/**
 * Weighted Jacobi smoother (omega = 2/3: optimal high-frequency damping).
 * Copies interior points only from x_temp back to x, preserving boundary
 * values in Dirichlet mode. BCs after each sweep.
 */
static void mg_smooth_jacobi(const mg_level_t* L, mg_bc_type_t bc,
        ...
        for (size_t k = L->k_start; k < L->k_end; k++) {
            for (size_t j = 1; j < ny - 1; j++) {
                for (size_t i = 1; i < nx - 1; i++) {
                    ...
                    x_temp[idx] = x[idx] + omega * (x_jacobi - x[idx]);
                }
            }
        }
...
static void mg_smooth(const mg_context_t* ctx, const mg_level_t* L,
                      double* x, const double* rhs, int sweeps) {
    if (ctx->smoother_type == MG_SMOOTHER_JACOBI) {
        mg_smooth_jacobi(L, ctx->bc_mode, x, L->x_temp, rhs, sweeps);
    } else {
        mg_smooth_rbgs(L, ctx->bc_mode, x, rhs, sweeps);
    }
}

/* ============================================================================
 * RESIDUAL AND GRID TRANSFERS
 * ============================================================================ */
 ...
 * reads them).
 */
static void mg_residual(const mg_level_t* L, const double* x,
 ...
 */
static void mg_restrict_level(const mg_context_t* ctx,
                              const mg_level_t* Lf, const mg_level_t* Lc,
                              const double* fine, double* coarse) {
    int fold = (ctx->bc_mode == MG_BC_NEUMANN);

    if (Lf->nz > 1) {
        mg_restrict_3d(fine, coarse, Lf->nx, Lf->ny, Lf->nz,
                       Lc->nx, Lc->ny, Lc->nz, fold);
    } else {
        mg_restrict_2d(fine, coarse, Lf->nx, Lf->ny, Lc->nx, Lc->ny, fold);
    }

    if (ctx->bc_mode == MG_BC_NEUMANN) {
        mg_subtract_interior_mean(coarse, Lc->nx, Lc->ny, Lc->nz);
    }
}

static void mg_prolongate_level(const mg_level_t* Lc, const mg_level_t* Lf,
                                const double* coarse, double* fine) {
    if (Lf->nz > 1) {
        mg_prolongate_add_3d(coarse, fine, Lc->nx, Lc->ny, Lc->nz,
                             Lf->nx, Lf->ny, Lf->nz);
    } else {
        mg_prolongate_add_2d(coarse, fine, Lc->nx, Lc->ny, Lf->nx, Lf->ny);
    }
}
 ...
 * negligible cost.
 */
static void mg_coarse_solve(const mg_context_t* ctx, const mg_level_t* L,
                            double* x, const double* rhs) {
    mg_smooth(ctx, L, x, rhs, ctx->coarse_max_iter);
}

/** Recursive V/W cycle. Level 0 is called with the caller's arrays. */
static void mg_cycle(mg_context_t* ctx, int level, double* x,
                     const double* rhs) {
    mg_level_t* L = &ctx->levels[level];

    if (level == ctx->num_levels - 1) {
        mg_coarse_solve(ctx, L, x, rhs);
        return;
    }
    ...
    mg_smooth(ctx, L, x, rhs, ctx->nu1);
    mg_residual(L, x, rhs, L->residual);
    mg_restrict_level(ctx, L, Lc, L->residual, Lc->rhs);
    ...
    for (int v = 0; v < num_visits; v++) {
        mg_cycle(ctx, level + 1, Lc->x, Lc->rhs);
    }
    ...
    if (ctx->bc_mode == MG_BC_NEUMANN) {
        ...
        mg_subtract_interior_mean(Lc->x, Lc->nx, Lc->ny, Lc->nz);
        mg_apply_bc_level(Lc, ctx->bc_mode, Lc->x);
    }

    mg_prolongate_level(Lc, L, Lc->x, x);
    mg_apply_bc_level(L, ctx->bc_mode, x);

    mg_smooth(ctx, L, x, rhs, ctx->nu2);
}
 ...
 */
static void mg_fmg(mg_context_t* ctx, double* x, const double* rhs) {
    int nl = ctx->num_levels;
    mg_level_t* levels = ctx->levels;

    if (nl == 1) {
        mg_cycle(ctx, 0, x, rhs);
        return;
    }

    /* Descent: levels[l].rhs = restricted original b for l >= 1 */
    mg_restrict_level(ctx, &levels[0], &levels[1], rhs, levels[1].rhs);
    for (int l = 1; l <= nl - 2; l++) {
        mg_restrict_level(ctx, &levels[l], &levels[l + 1],
                          levels[l].rhs, levels[l + 1].rhs);
    }

    /* Coarsest solve */
    memset(levels[nl - 1].x, 0, levels[nl - 1].total * sizeof(double));
    mg_coarse_solve(ctx, &levels[nl - 1], levels[nl - 1].x,
                    levels[nl - 1].rhs);

    /* Ascent through MG-owned levels */
    for (int l = nl - 2; l >= 1; l--) {
        mg_apply_bc_level(&levels[l + 1], ctx->bc_mode, levels[l + 1].x);
        memset(levels[l].x, 0, levels[l].total * sizeof(double));
        mg_prolongate_level(&levels[l + 1], &levels[l],
                            levels[l + 1].x, levels[l].x);
        mg_apply_bc_level(&levels[l], ctx->bc_mode, levels[l].x);
        mg_cycle(ctx, l, levels[l].x, levels[l].rhs);
    }
    ...
    mg_apply_bc_level(&levels[1], ctx->bc_mode, levels[1].x);
    {
        mg_level_t* L0 = &levels[0];
        for (size_t k = L0->k_start; k < L0->k_end; k++) {
            for (size_t j = 1; j < L0->ny - 1; j++) {
                for (size_t i = 1; i < L0->nx - 1; i++) {
                    x[k * L0->stride_z + IDX_2D(i, j, L0->nx)] = 0.0;
                }
            }
        }
    }
    mg_prolongate_level(&levels[1], &levels[0], levels[1].x, x);
    mg_apply_bc_level(&levels[0], ctx->bc_mode, x);
    mg_cycle(ctx, 0, x, rhs);
}
...
static void mg_free_levels(mg_context_t* ctx) {
    if (!ctx->levels) {
        return;
    }
...
static void mg_scalar_destroy(poisson_solver_t* solver) {
...
static cfd_status_t mg_scalar_init(
...
static cfd_status_t mg_scalar_iterate(
    poisson_solver_t* solver,
    double* x,
    double* x_temp,
    const double* rhs,
 ...
 */
static cfd_status_t mg_scalar_solve(
    poisson_solver_t* solver,
    double* x,
    double* x_temp,
    const double* rhs,
 ...
 */
static void mg_scalar_apply_bc(poisson_solver_t* solver, double* x) {
    mg_context_t* ctx = (mg_context_t*)solver->context;
    if (!ctx || !ctx->initialized) {
        return;
    }
    mg_apply_bc_level(&ctx->levels[0], ctx->bc_mode, x);
}
...
poisson_solver_t* create_multigrid_scalar_solver(void) {
    ...
    solver->name = POISSON_SOLVER_TYPE_MG_SCALAR;
    solver->description = "Geometric multigrid V/W/F-cycle (scalar CPU)";
    solver->method = POISSON_METHOD_MULTIGRID;
    solver->backend = POISSON_BACKEND_SCALAR;
    solver->params = poisson_solver_params_default();

    solver->init = mg_scalar_init;
    solver->destroy = mg_scalar_destroy;
    solver->solve = mg_scalar_solve;      /* Resets FMG state, then common loop */
    solver->iterate = mg_scalar_iterate;  /* One V/W cycle (or the FMG pass) */
    solver->apply_bc = mg_scalar_apply_bc;
