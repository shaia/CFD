...
__global__ void kernel_poisson_jacobi(const double* __restrict__ p_old,
...
__global__ void kernel_poisson_sor_rb(double* __restrict__ p,
                                       const double* __restrict__ rhs,
                                       size_t nx, size_t ny,
                                       double inv_dx2, double inv_dy2,
                                       double omega, double inv_factor,
                                       int color,
    ...
    if (i < nx - 1 && j < ny - 1 && (i + j) % 2 == color) {
        ...
        if (residual_out) {
            double lap = (p[idx+1] - 2.0*p_c + p[idx-1]) * inv_dx2 + (p[idx+nx] - 2.0*p_c + p[idx-nx]) * inv_dy2;
    ...
    if (residual_out) {
        ...
        for (int s = blockDim.x * blockDim.y / 2; s > 0; s >>= 1) {
            if (tid < s) s_residual[tid] = fmax(s_residual[tid], s_residual[tid + s]);
            __syncthreads();
        }
        if (tid == 0) atomicMax((unsigned long long*)residual_out, __double_as_longlong(s_residual[0]));
    }
}

// Predictor step - compute u* without pressure
__global__ void kernel_predictor(const double* __restrict__ u,
                                  const double* __restrict__ v,
    ...
    if (i < nx - 1 && j < ny - 1) {
        ...
        double d2u = (u[idx+1] - 2.0*u_c + u[idx-1]) * inv_dx2 + (u[idx+nx] - 2.0*u_c + u[idx-nx]) * inv_dy2;
        double d2v = (v[idx+1] - 2.0*v_c + v[idx-1]) * inv_dx2 + (v[idx+nx] - 2.0*v_c + v[idx-nx]) * inv_dy2;
...
__global__ void kernel_velocity_rhs(const double* __restrict__ u,
                                     const double* __restrict__ v,
                                     const double* __restrict__ p,
                                     double* __restrict__ u_rhs,
                                     double* __restrict__ v_rhs,
                                     size_t nx, size_t ny,
                                     double inv_2dx, double inv_2dy,
                                     double inv_dx2, double inv_dy2,
...
__global__ void kernel_velocity_update(double* __restrict__ u,
                                        double* __restrict__ v,
                                        const double* __restrict__ u_rhs,
                                        const double* __restrict__ v_rhs,
                                        size_t nx, size_t ny, double dt) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i < nx - 1 && j < ny - 1) {
        size_t idx = j * nx + i;
...
__global__ void kernel_projection_correct(double* __restrict__ u,
                                           double* __restrict__ v,
...
__global__ void kernel_pressure_update(double* __restrict__ p,
                                        const double* __restrict__ div,
...
__global__ void kernel_scale_rhs(double* __restrict__ rhs, size_t nx, size_t ny, double scale) {
    int i = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int j = blockIdx.y * blockDim.y + threadIdx.y + 1;
    if (i < nx - 1 && j < ny - 1) rhs[j * nx + i] *= scale;
}
...
int gpu_get_device_info(GPUDeviceInfo* info, int max_devices) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (cudaGetDeviceProperties(&prop, i) == cudaSuccess) {
            ...
            cudaSetDevice(i);
            info[i].free_memory = (cudaMemGetInfo(&free_mem, &total_mem) == cudaSuccess) ? free_mem : 0;
        }
    }
    return count;
}
...
void gpu_solver_destroy(GPUSolverContext* ctx) {
    ...
    if (ctx->memory_allocated) {
        cudaFree(ctx->d_u); cudaFree(ctx->d_v); cudaFree(ctx->d_p); cudaFree(ctx->d_rho);
        cudaFree(ctx->d_u_new); cudaFree(ctx->d_v_new); cudaFree(ctx->d_p_new);
        cudaFree(ctx->d_rhs); cudaFree(ctx->d_residual);
        cudaFree(ctx->d_x); cudaFree(ctx->d_y); cudaFree(ctx->d_dx); cudaFree(ctx->d_dy);
    }
...
int gpu_solver_step(GPUSolverContext* ctx, const Grid* grid, const SolverParams* params, GPUSolverStats* stats) {
    if (!ctx || !grid || !params) return -1;
    size_t nx = ctx->nx, ny = ctx->ny;
    double dx = grid->dx[0], dy = grid->dy[0], dt = params->dt, nu = params->mu;
    double inv_2dx = 0.5/dx, inv_2dy = 0.5/dy, inv_dx2 = 1.0/(dx*dx), inv_dy2 = 1.0/(dy*dy);
    ...
    kernel_velocity_rhs<<<grid_dim, block, 0, ctx->stream>>>(ctx->d_u, ctx->d_v, ctx->d_p, ctx->d_u_new, ctx->d_v_new, nx, ny, inv_2dx, inv_2dy, inv_dx2, inv_dy2, nu, 1.0);
    kernel_velocity_update<<<grid_dim, block, 0, ctx->stream>>>(ctx->d_u, ctx->d_v, ctx->d_u_new, ctx->d_v_new, nx, ny, dt);
    kernel_apply_boundary_velocity<<<bb, 256, 0, ctx->stream>>>(ctx->d_u, ctx->d_v, nx, ny);
    kernel_compute_divergence<<<grid_dim, block, 0, ctx->stream>>>(ctx->d_u, ctx->d_v, ctx->d_rhs, nx, ny, inv_2dx, inv_2dy);
    kernel_pressure_update<<<grid_dim, block, 0, ctx->stream>>>(ctx->d_p, ctx->d_rhs, nx, ny, 0.1*dt/(dx*dx));
...
GPUSolverStats gpu_solver_get_stats(const GPUSolverContext* ctx) {
    if (!ctx) { GPUSolverStats e = {0}; return e; }
    return ctx->stats;
}
...
void solve_navier_stokes_gpu(FlowField* field, const Grid* grid, const SolverParams* params, const GPUConfig* config) {
    if (!field || !grid || !params) return;
    GPUConfig cfg = config ? *config : gpu_config_default();
    if (!gpu_should_use(&cfg, field->nx, field->ny, params->max_iter)) return;
    GPUSolverContext* ctx = gpu_solver_create(field->nx, field->ny, &cfg);
    if (!ctx) return;
    if (gpu_solver_upload(ctx, field) != 0) { gpu_solver_destroy(ctx); return; }
    GPUSolverStats stats;
    for (int iter = 0; iter < params->max_iter; iter++) {
        if (gpu_solver_step(ctx, grid, params, &stats) != 0) break;
    }
    gpu_solver_download(ctx, field);
    gpu_solver_destroy(ctx);
}

void solve_projection_method_gpu(FlowField* field, const Grid* grid, const SolverParams* params, const GPUConfig* config) {
    if (!field || !grid || !params) return;
    GPUConfig cfg = config ? *config : gpu_config_default();
    if (!gpu_should_use(&cfg, field->nx, field->ny, params->max_iter)) return;
    ...
    for (int iter = 0; iter < params->max_iter; iter++) {
        ...
        kernel_predictor<<<grid_dim, block, 0, ctx->stream>>>(ctx->d_u, ctx->d_v, ctx->d_u_new, ctx->d_v_new, nx, ny, dt, nu, inv_2dx, inv_2dy, inv_dx2, inv_dy2);
        kernel_apply_boundary_velocity<<<bb, 256, 0, ctx->stream>>>(ctx->d_u_new, ctx->d_v_new, nx, ny);

        // Step 2: Compute divergence of u*
        kernel_compute_divergence<<<grid_dim, block, 0, ctx->stream>>>(ctx->d_u_new, ctx->d_v_new, ctx->d_rhs, nx, ny, inv_2dx, inv_2dy);
        ...
        for (int pi = 0; pi < cfg.poisson_max_iter; pi++) {
            kernel_poisson_jacobi<<<grid_dim, block, 0, ctx->stream>>>(p_src, p_dst, ctx->d_rhs, nx, ny, inv_dx2, inv_dy2, inv_factor);
            kernel_apply_boundary_pressure<<<bb, 256, 0, ctx->stream>>>(p_dst, nx, ny);
            double* tmp = p_src; p_src = p_dst; p_dst = tmp;
        }
        // Ensure result is in d_p
        if (p_src != ctx->d_p) {
            cudaMemcpyAsync(ctx->d_p, p_src, nx*ny*sizeof(double), cudaMemcpyDeviceToDevice, ctx->stream);
        }

        // Step 4: Corrector - project velocity
        kernel_projection_correct<<<grid_dim, block, 0, ctx->stream>>>(ctx->d_u, ctx->d_v, ctx->d_u_new, ctx->d_v_new, ctx->d_p, nx, ny, dt_rho, inv_2dx, inv_2dy);
        kernel_apply_boundary_velocity<<<bb, 256, 0, ctx->stream>>>(ctx->d_u, ctx->d_v, nx, ny);
    }
