...
static void write_velocity_vtk(const char* run_dir, const char* prefix, int step,
    ...
    write_vtk_vector_output(filepath, "velocity", field->u, field->v, field->w, grid->nx, grid->ny,
                            grid->nz, grid->xmin, grid->xmax, grid->ymin, grid->ymax, grid->zmin,
                            grid->zmax);
}
...
static void write_full_field_vtk(const char* run_dir, const char* prefix, int step,
    ...
    write_vtk_flow_field(filepath, field, grid->nx, grid->ny, grid->nz, grid->xmin, grid->xmax,
                         grid->ymin, grid->ymax, grid->zmin, grid->zmax);
}
...
void vtk_write_scalar_field(const char* run_dir, const char* prefix, int step,
    ...
    write_vtk_output(filepath, field_name, data, grid->nx, grid->ny, grid->nz, grid->xmin,
                     grid->xmax, grid->ymin, grid->ymax, grid->zmin, grid->zmax);
}
...
void write_vtk_vector_output(const char* filename, const char* field_name, const double* u_data,
    ...
    for (size_t k = 0; k < nz; k++) {
        for (size_t j = 0; j < ny; j++) {
            for (size_t i = 0; i < nx; i++) {
                size_t idx = (k * nx * ny) + IDX_2D(i, j, nx);
                double w_val = w_data ? w_data[idx] : 0.0;
