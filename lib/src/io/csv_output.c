...
static void write_timeseries_csv(const char* run_dir, const char* prefix, int step,
    ...
    write_csv_timeseries(filepath, step, current_time, field, params, stats, grid->nx, grid->ny,
                         create_new);
}
...
static void write_centerline_csv(const char* run_dir, const char* prefix, int step,
    ...
    write_csv_centerline(filepath, field, grid->x, grid->y, grid->nx, grid->ny, PROFILE_HORIZONTAL);
}
...
static void write_statistics_csv(const char* run_dir, const char* prefix, int step,
    ...
    int create_new = (step == 0);
    write_csv_statistics(filepath, step, current_time, field, grid->nx, grid->ny, create_new);
}
...
void csv_dispatch_output(CsvOutputType csv_type, const char* run_dir, const char* prefix, int step,
    ...
    if (csv_type < CSV_OUTPUT_TABLE_SIZE) {
        csv_output_table[csv_type](run_dir, prefix, step, current_time, field, grid, params, stats);
    } else {
...
void write_csv_timeseries(const char* filename, int step, double time, const FlowField* field,
    ...
    if (!filename || !field || !params || !stats)
        return;
    ...
    if (write_header) {
        fprintf(
            fp,
            "step,time,dt,max_u,max_v,max_p,avg_u,avg_v,avg_p,iterations,residual,elapsed_ms\n");
    }
    ...
    fprintf(fp, "%d,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%d,%.6e,%.2f\n", step, time, params->dt,
            u_stats.max_val, v_stats.max_val, p_stats.max_val, u_stats.avg_val, v_stats.avg_val,
            p_stats.avg_val, stats->iterations, stats->residual, stats->elapsed_time_ms);
...
void write_csv_statistics(const char* filename, int step, double time, const FlowField* field,
    ...
    // Write header if new file
    if (write_header) {
        fprintf(fp, "step,time,min_u,max_u,avg_u,min_v,max_v,avg_v,min_p,max_p,avg_p,min_rho,max_"
                    "rho,avg_rho,min_T,max_T,avg_T\n");
    }
    ...
    // Write data row
    fprintf(fp,
            "%d,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e\n",
            step, time, u_stats.min_val, u_stats.max_val, u_stats.avg_val, v_stats.min_val,
            v_stats.max_val, v_stats.avg_val, p_stats.min_val, p_stats.max_val, p_stats.avg_val,
            rho_stats.min_val, rho_stats.max_val, rho_stats.avg_val, T_stats.min_val,
            T_stats.max_val, T_stats.avg_val);
