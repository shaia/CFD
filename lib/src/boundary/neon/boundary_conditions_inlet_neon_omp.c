...
static inline void inlet_get_base_velocity_neon(const bc_inlet_config_t* config,
...
static inline void inlet_apply_profile_neon(const bc_inlet_config_t* config,
                                             double u_base, double v_base,
                                             double position,
...
static inline void inlet_compute_velocity_neon(const bc_inlet_config_t* config, double position,
                                                double* u_out, double* v_out) {
    double u_base, v_base;
    inlet_get_base_velocity_neon(config, &u_base, &v_base);
    inlet_apply_profile_neon(config, u_base, v_base, position, u_out, v_out);
}
...
#pragma omp parallel for schedule(static)
    for (idx = 0; idx < size_to_int(count); idx++) {
        ...
        double u_val, v_val;
        inlet_compute_velocity_neon(config, position, &u_val, &v_val);
