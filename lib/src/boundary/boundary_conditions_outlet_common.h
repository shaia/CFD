 ...
 * Back:   field[k=0 plane] = field[k=1 plane]      (3D only)
 * Front:  field[k=nz-1 plane] = field[k=nz-2 plane] (3D only)
 * ============================================================================ */
 ...
 */
typedef struct {
    bc_outlet_idx_func_t dst_fn;        /* Boundary index computation (NULL for z-faces) */
    bc_outlet_idx_func_t src_fn;        /* Interior index computation (NULL for z-faces) */
 ...
 */
static const bc_outlet_edge_loop_t bc_outlet_edge_loops[6] = {
    { .dst_fn = bc_outlet_dst_left,   .src_fn = bc_outlet_src_left,   .use_ny_for_count = 1, .is_z_face = 0 },
    { .dst_fn = bc_outlet_dst_right,  .src_fn = bc_outlet_src_right,  .use_ny_for_count = 1, .is_z_face = 0 },
    { .dst_fn = bc_outlet_dst_bottom, .src_fn = bc_outlet_src_bottom, .use_ny_for_count = 0, .is_z_face = 0 },
    { .dst_fn = bc_outlet_dst_top,    .src_fn = bc_outlet_src_top,    .use_ny_for_count = 0, .is_z_face = 0 },
    { .dst_fn = NULL,                 .src_fn = NULL,                 .use_ny_for_count = 0, .is_z_face = 1 },  /* FRONT */
    { .dst_fn = NULL,                 .src_fn = NULL,                 .use_ny_for_count = 0, .is_z_face = 1 },  /* BACK */
};
