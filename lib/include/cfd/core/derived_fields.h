...
CFD_LIBRARY_EXPORT field_stats calculate_field_statistics(const double* data, size_t count);
...
CFD_LIBRARY_EXPORT void derived_fields_compute_velocity_magnitude(derived_fields* derived,
...
CFD_LIBRARY_EXPORT void derived_fields_compute_statistics(derived_fields* derived,
                                                          const flow_field* field);

// Clear all computed fields (frees memory, sets pointers to NULL, resets stats)
CFD_LIBRARY_EXPORT void derived_fields_clear(derived_fields* derived);
