...
static ns_solver_params_t make_nondefault_params(void) {
    ...
    p.pressure_bc.left = POISSON_WALL_DIRICHLET;
    p.pressure_bc.right = POISSON_WALL_DIRICHLET;
    p.pressure_bc.bottom = POISSON_WALL_ZERO_GRADIENT;
    p.pressure_bc.top = POISSON_WALL_ZERO_GRADIENT;
    p.pressure_bc.front = POISSON_WALL_DIRICHLET;
    p.pressure_bc.back = POISSON_WALL_ZERO_GRADIENT;
    p.pressure_bc.values.left = 0.0;
    p.pressure_bc.values.right = -6.4;
    p.pressure_bc.values.top = 1.5;
    p.pressure_bc.values.bottom = -1.5;
    p.pressure_bc.values.front = 2.5;
    p.pressure_bc.values.back = -2.5;
...
static void assert_params_equal(const ns_solver_params_t* a, const ns_solver_params_t* b) {
    ...
    TEST_ASSERT_EQUAL_INT(a->pressure_bc.left, b->pressure_bc.left);
    TEST_ASSERT_EQUAL_INT(a->pressure_bc.right, b->pressure_bc.right);
    TEST_ASSERT_EQUAL_INT(a->pressure_bc.bottom, b->pressure_bc.bottom);
    TEST_ASSERT_EQUAL_INT(a->pressure_bc.top, b->pressure_bc.top);
    TEST_ASSERT_EQUAL_INT(a->pressure_bc.front, b->pressure_bc.front);
    TEST_ASSERT_EQUAL_INT(a->pressure_bc.back, b->pressure_bc.back);
    assert_bc_values_equal(&a->pressure_bc.values, &b->pressure_bc.values);
...
void test_reject_unknown_nut_correction(void) {
    ...
    /* The same door covers viscous_scheme */
    ns_solver_params_t v = ns_solver_params_default();
