/**
 * Validation Tests for OpenMP-Parallelized RK4 Solver
 *
 * Tests that OpenMP RK4 implementation produces results consistent
 * with the serial RK4 implementation.
 */
...
//=============================================================================
// TEST: OPENMP RK4 CREATES
//=============================================================================

void test_omp_rk4_creates(void) {
    printf("\n=== Test: OpenMP RK4 Creates ===\n");
    ...
    ns_solver_t* slv = cfd_solver_create(registry, NS_SOLVER_TYPE_RK4_OMP);
    if (slv == NULL) {
        printf("OpenMP RK4 solver not available (expected if OpenMP disabled)\n");
...
void test_omp_rk4_scalar_consistency(void) {
    printf("\n=== Test: OpenMP RK4 vs Scalar Consistency ===\n");

    ns_solver_registry_t* registry = cfd_registry_create();
    cfd_registry_register_defaults(registry);

    if (!omp_solver_available(registry, NS_SOLVER_TYPE_RK4_OMP)) {
        printf("OpenMP RK4 solver not available, skipping consistency test\n");
        cfd_registry_destroy(registry);
        TEST_PASS();
        return;
    }
...
void test_omp_rk4_stability(void) {
    printf("\n=== Test: OpenMP RK4 Stability with Large Grid ===\n");

    ns_solver_registry_t* registry = cfd_registry_create();
    cfd_registry_register_defaults(registry);

    if (!omp_solver_available(registry, NS_SOLVER_TYPE_RK4_OMP)) {
        printf("OpenMP RK4 solver not available, skipping\n");
        cfd_registry_destroy(registry);
        TEST_PASS();
        return;
    }
    ...
    test_result result = test_run_stability(NS_SOLVER_TYPE_RK4_OMP, 128, 128, &params, 100);

    TEST_ASSERT_TRUE_MESSAGE(result.passed, result.message);
    printf("OMP RK4 solver stable for 100 steps on 128x128 grid\n");
    printf("PASSED\n");
}
...
void test_omp_rk4_deterministic(void) {
    printf("\n=== Test: OpenMP RK4 Deterministic Results ===\n");
    ...
    ns_solver_t* slv1 = cfd_solver_create(registry, NS_SOLVER_TYPE_RK4_OMP);
    ns_solver_t* slv2 = cfd_solver_create(registry, NS_SOLVER_TYPE_RK4_OMP);
    ...
    TEST_ASSERT_TRUE_MESSAGE(u_diff < 1e-10, "OMP RK4 results should be deterministic");
    TEST_ASSERT_TRUE_MESSAGE(v_diff < 1e-10, "OMP RK4 results should be deterministic");
...
void test_omp_rk4_energy_decay(void) {
    printf("\n=== Test: OpenMP RK4 Energy Decay ===\n");
    ...
    test_result result = test_run_energy_decay(NS_SOLVER_TYPE_RK4_OMP, 32, 32, &params, 50);
...
int main(void) {
    ...
    printf("\n");
    printf("================================================\n");
    printf("  OpenMP RK4 NSSolver Tests\n");
    printf("================================================\n");
    ...
    RUN_TEST(test_omp_rk4_creates);
    RUN_TEST(test_omp_rk4_scalar_consistency);
    RUN_TEST(test_omp_rk4_stability);
    RUN_TEST(test_omp_rk4_deterministic);
    RUN_TEST(test_omp_rk4_energy_decay);
