/**
 * Mathematical Validation Tests for the GPU RK4 (classical Runge-Kutta) Solver
 *
 * Verifies that the rk4_gpu backend matches the scalar rk4 reference within
...
static int gpu_solver_available(void) {
    ...
    ns_solver_t* slv = cfd_solver_create(registry, NS_SOLVER_TYPE_RK4_GPU);
...
void test_gpu_solver_creates(void) {
    ...
    ns_solver_t* slv = cfd_solver_create(registry, NS_SOLVER_TYPE_RK4_GPU);
    if (slv == NULL) {
        printf("RK4 GPU solver not available (expected if CUDA not enabled)\n");
        cfd_registry_destroy(registry);
        TEST_PASS();
        return;
    }
...
void test_gpu_cpu_consistency(void) {
    printf("\n=== Test: RK4 GPU vs CPU Consistency ===\n");

    if (!gpu_is_available() || !gpu_solver_available()) {
        printf("RK4 GPU solver not available, skipping\n");
        TEST_PASS();
        return;
    }
    ...
    test_result result = test_run_consistency(
        NS_SOLVER_TYPE_RK4, NS_SOLVER_TYPE_RK4_GPU, 32, 32, &params, 10, 0.10);
...
void test_gpu_cpu_consistency_energy(void) {
    printf("\n=== Test: RK4 GPU vs CPU Consistency (energy on) ===\n");
...
void test_gpu_stability(void) {
    ...
    test_result result = test_run_stability(NS_SOLVER_TYPE_RK4_GPU, 64, 64, &params, 50);

    TEST_ASSERT_TRUE_MESSAGE(result.passed, result.message);
    printf("RK4 GPU solver stable for 50 steps\n");
    printf("PASSED\n");
}

void test_gpu_energy_decay(void) {
    printf("\n=== Test: RK4 GPU Energy Decay ===\n");
    ...
    test_result result = test_run_energy_decay(NS_SOLVER_TYPE_RK4_GPU, 32, 32, &params, 30);
...
int main(void) {
    ...
    printf("\n");
    printf("================================================\n");
    printf("  GPU RK4 Solver Validation Tests\n");
    printf("================================================\n");
