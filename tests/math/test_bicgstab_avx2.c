/**
 * @file test_bicgstab_avx2.c
 * @brief Consistency test: BiCGSTAB AVX2 vs Scalar
 *
 * Verifies that the AVX2-optimized BiCGSTAB solver produces identical results
...
/**
 * Test: BiCGSTAB AVX2 vs Scalar Consistency
 *
 * Solves the same Poisson problem with both scalar and AVX2 BiCGSTAB solvers,
 * then verifies the solutions are numerically identical.
 */
void test_bicgstab_avx2_scalar_consistency(void) {
    ...
    status = poisson_solver_init(solver_avx2, NX, NY, dx, dy, &params);
    TEST_ASSERT_EQUAL(CFD_SUCCESS, status);

    /* Solve with AVX2 solver */
...
int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_bicgstab_avx2_scalar_consistency);
    return UNITY_END();
}
