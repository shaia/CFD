...
void test_gmres_omp_vs_scalar(void) {
    ...
    poisson_solver_t* solver_omp = poisson_solver_create(
        POISSON_METHOD_GMRES, POISSON_BACKEND_OMP);
    ...
    if (!solver_omp) {
        ...
        cfd_free(rhs);
        poisson_solver_destroy(solver_scalar);
        TEST_IGNORE_MESSAGE("OMP GMRES solver creation failed; backend may be unavailable");
        return;
    }
    ...
    /* GMRES OMP and scalar share byte-identical dense math; only the
     * parallel-reduction dot products differ, so agreement is excellent. */
    TEST_ASSERT_DOUBLE_WITHIN(1.0e-9, 0.0, l2_diff);
